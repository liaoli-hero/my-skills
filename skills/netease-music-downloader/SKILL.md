...
- 离线自测：`python -m unittest discover -s tests -v`（纯标准库，23 个用例覆盖 MP3 头校验/VIP 识别/文件名安全/占位图结构/ID3 mime/歌单解析/进度格式/幂等判定）
...
5. **离线自测**：改脚本后跑 `python -m unittest discover -s tests -v`，23 个用例防回归
...
4. **改动即测**：任何脚本改动必须通过 `tests/` 23 个离线用例（防回归，v1.0.3 曾靠自测抓出真实 bug）。
...
**Q: 重复运行会重复下载吗？**
A: 不会。脚本是幂等的——目标文件已存在且是完整 mp3 时跳过下载，只自动补缺失的歌词/封面。同一歌单可以安全地反复执行。

**Q: 下载时没有进度条？**
A: 进度条只在真实终端（TTY）显示；输出被重定向到文件或管道时自动隐藏，避免污染日志——这是设计行为，不是故障。
